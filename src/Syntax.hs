...
mergeCC :: Expression -> Expression
mergeCC (IntersectionCasts cs (IntersectionCasts cs' expr)) =
    ...
    let intersectionCasts = concatMap (\x -> concatMap (\y -> [joinCastI y x | isSameCastLabel y x]) cs) cs'
...
mergeCI (IntersectionCasts cs (Cast t1 t2 expr)) =
    ...
    -- join the single cast with each intersection cast
    in IntersectionCasts (map (\x -> joinCastI x (head cs')) cs) expr'
