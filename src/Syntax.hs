...
convertToSimpleCast (BlameCast _ _ f msg) expr = Blame f msg
...
mergeII :: Expression -> Expression
mergeII (IntersectionCasts cs (IntersectionCasts cs' expr)) =
    ...
    let intersectionCasts = concatMap (\x -> concatMap (\y -> [joinCastI y x | isSameCastLabel y x && initialType y == finalType x]) cs) cs'
    in IntersectionCasts intersectionCasts expr

-- Merge intersection casts with casts
mergeIC :: Expression -> Expression
mergeIC (Cast t1 t2 (IntersectionCasts cs expr)) =
    -- append single cast at the head of every casts in intersection of casts
    let intersectionCasts = concatMap (\x -> [SingleCast (getCastLabel x) t1 t2 x | t1 == finalType x]) cs
    in IntersectionCasts intersectionCasts expr

-- Merge cast with intersection cast
mergeCI :: Expression -> Expression
mergeCI (IntersectionCasts cs (Cast t1 t2 expr)) =
    -- append single cast at the tail of every casts in intersection of casts
    let (IntersectionCasts cs' expr') = convertToIntersectionCasts $ Cast t1 t2 expr
        intersectionCasts = concatMap (\x -> [joinCastI x (head cs') | t2 == initialType x]) cs
    in IntersectionCasts intersectionCasts expr
...
getCompatibleArrowType (EmptyCast cl (ArrowType t1 t2)) = (EmptyCast cl t1, EmptyCast cl t2)
